...
private[internal] trait Transform { self: QuoteImpl =>
  ...
  import self.c.{typeTag, Type}
  private val StringTpe = typeTag[String].tpe
  private val SeqOfNodeTpe = typeTag[Seq[scala.xml.Node]].tpe
  private val OptionOfSeqOfNodeTpe = typeTag[Option[Seq[scala.xml.Node]]].tpe
